...
function DispatchRequests() {
  ...
  const sendDispatch = useCallback(async (marksheet) => {
    ...
    try {
      ...
      let data
      try {
        data = await apiClient.post(`${origin}/api/whatsapp-dispatch?action=send-marksheet`, payload)
      } catch (apiErr) {
    ...
    try {
      ...
      for (const marksheet of approvedMarksheets) {
        try {
          ...
          try {
            responseData = await apiClient.post(`${origin}/api/whatsapp-dispatch?action=send-marksheet`, { marksheetId: marksheet._id, marksheetPdfUrl, marksheetImageUrl })
          } catch (apiErr) {
