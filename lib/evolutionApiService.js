...
class EvolutionAPIService {
  ...
  async deleteInstance() {
    ...
    try {
      console.log([Evolution] Attempting to delete instance: );

      // Fast path: try force delete first so connected/connecting instances do not wait on logout.
      try {
        console.log('[Evolution] Attempting fast force delete...');
        const response = await this.client.delete(/instance/delete/?force=true, {
          timeout: 12000
        });
      ...
      // Fallback: try a short logout, then regular delete.
      try {
        console.log('[Evolution] Attempting fallback logout...');
        await this.client.delete(/instance/logout/, {
          timeout: 8000
        });
        console.log('[Evolution] Fallback logout succeeded');
      } catch (logoutErr) {
        console.log('[Evolution] Fallback logout failed, continuing to delete:', logoutErr.message);
      }

      try {
        console.log('[Evolution] Attempting regular delete...');
        const response = await this.client.delete(/instance/delete/, {
          timeout: 12000
        });
