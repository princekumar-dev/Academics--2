...
// Coalesce simultaneous QR/create requests for the same staff instance. This
// prevents two browser tabs (or retries) from creating competing Baileys
// sessions for one WhatsApp number.
...
class EvolutionAPIService {
  ...
  constructor(options = {}) {
    ...
    // Prevent a logged-out instance from being hit repeatedly by a batch/job.
    // A WhatsApp `device_removed` logout cannot be silently re-authenticated: it
    // requires the account owner to scan a new QR code.
    this.connectionBlockedUntil = 0;
  ...
  async createInstance(qrcode = true) {
    const existing = instanceCreationPromises.get(this.instanceName);
    ...
    const creation = (async () => {
      try {
        ...
        return { success: true, data: response.data, qrcode: response.data.qrcode };
      } catch (error) {
        console.error('Failed to create instance:', error.message);
        return { success: false, error: error.message };
      } finally {
        instanceCreationPromises.delete(this.instanceName);
      }
    })();

    instanceCreationPromises.set(this.instanceName, creation);
    return creation;
  }
   ...
   * @returns {Object} - Connection diagnostics info
   */
  ...
  /**
   * Check if an error indicates a connection problem (device_removed, connection closed, etc.)
   * @param {any} err - Error object
   * @returns {boolean}
   */
  isConnectionError(err) {
    ...
    return (
      ...
      errorData.includes('device_removed') ||
      errorData.includes('conflict') ||
      err?.response?.status === 428 || // Precondition Required
      err?.code === 'ECONNRESET' ||
  ...
  /**
   * Create a descriptive connection error
   * @param {any} originalError - The original error
   * @returns {Error}
   */
  createConnectionError(originalError) {
    const error = new Error(
      'WhatsApp connection lost (device_removed). The QR code needs to be scanned again to reconnect.'
    );
    error.code = 'DEVICE_REMOVED';
    error.isConnectionError = true;
    error.originalError = originalError;
    return error;
  }

  blockConnectionAttempts() {
    // Keep a short circuit breaker in this server process. Status checks remain
    // available, but dispatch attempts do not produce a cascade of provider calls.
    this.connectionBlockedUntil = Date.now() + 60 * 1000;
  }

  /**
   * Check if the WhatsApp connection is alive before sending
   * @returns {boolean} - True if connected, false otherwise
   */
  async isConnected() {
    if (Date.now() < this.connectionBlockedUntil) return false;
    try {
      const status = await this.getInstanceStatus();
      return status.success && (status.connected || status.state === 'open');
    } catch {
      return false;
    }
  }
  ...
  async sendTextMessage(phoneNumber, message) {
    ...
    if (!connected) {
      const error = new Error('WhatsApp instance is not connected. Please reconnect by scanning the QR code again.');
    ...
    try {
      ...
      for (const number of numberVariants) {
        // Evolution's sendText endpoint requires this top-level `text` value.
        // Do not retry alternate schemas: each rejected schema was previously
        // creating another "instance requires property text" error.
        ...
        try {
          ...
          if (this.isConnectionError(attemptErr)) {
            this.blockConnectionAttempts();
            throw this.createConnectionError(attemptErr);
          }
        }
      }
      ...
      throw lastError || new Error('Unable to send WhatsApp text message');
        ...
        // If already a connection error, rethrow as-is
        ...
        if (error && error.response) {
          ...
          if (this.isConnectionError(error)) {
            const connErr = this.createConnectionError(error);
            connErr.response = error.response;
            throw connErr;
          }
    ...
    try {
      ...
      for (const number of numberVariants) {
        ...
        try {
          ...
          // If we get a connection error, don't retry with other number variants
          if (this.isConnectionError(attemptErr)) {
            this.blockConnectionAttempts();
            throw this.createConnectionError(attemptErr);
          }
  ...
  async sendMarksheetNotification({
    ...
    // Check once before the document/text sequence. If WhatsApp is logged out,
    // do not start a partial dispatch at all.
    if (!(await this.isConnected())) {
      const error = new Error('WhatsApp instance is not connected. Reconnect it by scanning the QR code before dispatching marksheets.');
      error.code = 'INSTANCE_NOT_CONNECTED';
      error.isConnectionError = true;
      throw error;
    }
...
_This is an automated message_`;
      ...
      if (pdfUrl) {
        try {
          ...
          // A provider logout makes the following text request guaranteed to fail.
          // Stop this marksheet and let the UI/job report that reconnection is needed.
          if (pdfError.isConnectionError || pdfError.code === 'DEVICE_REMOVED' || pdfError.code === 'INSTANCE_NOT_CONNECTED') {
            throw pdfError;
          }
          // Non-connection media failures may still receive the text notification.
        }
  ...
  async sendBulkMessages(messages, delayMs = 2000) {
    ...
    // Check connection once before starting bulk send
    const connected = await this.isConnected();
    if (!connected) {
      const error = new Error('WhatsApp instance is not connected. Cannot send bulk messages.');
      error.code = 'INSTANCE_NOT_CONNECTED';
      error.isConnectionError = true;
      throw error;
    }
    ...
    for (const msg of messages) {
      try {
        ...
        // If connection error, stop sending remaining messages
        if (error.isConnectionError) {
          console.error('Connection lost during bulk send, stopping remaining messages');
          // Mark remaining messages as failed
          const remainingIndex = messages.indexOf(msg) + 1;
          for (let i = remainingIndex; i < messages.length; i++) {
            results.push({
              phoneNumber: messages[i].phoneNumber,
              success: false,
              error: 'Skipped: WhatsApp connection lost'
            });
          }
          break;
        }
